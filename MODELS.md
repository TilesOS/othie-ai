...
`configured` is a deployment-supplied revision label. Production operators should replace it with the exact Ollama artifact digest/revision they deploy and preserve the associated license and notice files. Kith includes provider, model, revision, dimensions, and preprocessing identity in derived-data and cache invalidation boundaries; it never combines vector tables across identities.
