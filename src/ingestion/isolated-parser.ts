...
export async function parseDocumentIsolated(path:string,maxBytes:number):Promise<ParsedSection[]>{
  if(![".pdf",".docx"].includes(extname(path).toLowerCase()))return parseDocument(path,maxBytes);
