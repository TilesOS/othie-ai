...
export async function startBridge():Promise<McpServer>{
  const configPath=resolve(process.env.KITH_CONFIG??process.argv[2]??"config.json");const config=await loadConfig(configPath);const dataDir=config.data_dir??defaultDataDir();const ipcPath=config.ipc_path??defaultIpcPath(dataDir);
  const bridgeId=process.env.KITH_BRIDGE_ID;if(!bridgeId)throw new Error("KITH_BRIDGE_ID is required");let credential=process.env.KITH_BRIDGE_CREDENTIAL;if(!credential&&process.env.KITH_BRIDGE_CREDENTIAL_FILE)credential=(await readFile(process.env.KITH_BRIDGE_CREDENTIAL_FILE,"utf8")).trim();if(!credential)throw new Error("KITH_BRIDGE_CREDENTIAL or KITH_BRIDGE_CREDENTIAL_FILE is required");
  const server=new McpServer({name:"kith-ai",version:"0.1.0"});
  ...
  server.registerTool("get_context_status",{title:"Get Kith context status",description:"Read operational indexing and revision status without document contents.",inputSchema:z.object({})},async()=>{
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href)startBridge().catch((error)=>{process.stderr.write(`Kith bridge failed: ${error instanceof Error?error.message:String(error)}\n`);process.exitCode=1;});
