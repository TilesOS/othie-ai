...
export class KithEngine{
  ...
  constructor(readonly config:KithConfig,readonly dataDir:string){this.store=new ManifestStore(join(dataDir,"manifest.sqlite"));this.lance=new LanceIndex(dataDir);this.providers=new ProviderRegistry(config);this.ingestion=new IngestionManager(config,dataDir,this.store,this.lance,this.providers);this.compiler=new ContextCompiler(config,this.store,this.lance,this.providers);}
  async start():Promise<void>{await mkdir(this.dataDir,{recursive:true,mode:0o700});const identity=JSON.stringify({parser:this.config.parser_version,chunker:this.config.chunker_version,embedding:this.config.models.embedding,compiler:this.config.models.compiler});if(this.store.getMetadata("build_identity")!==identity){this.ingestion.rebuild();this.store.setMetadata("build_identity",identity);}await this.ingestion.start();}
  async stop():Promise<void>{await this.ingestion.stop();this.lance.close();this.store.close();}
  ...
  async purge():Promise<void>{this.store.purge();await this.lance.purge();}
}
